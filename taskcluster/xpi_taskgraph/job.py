...
yarn_schema = Schema(
    {
        Required("using"): "yarn",
        Required("yarn"): [text_type],
        Optional("post-yarn"): [[text_type]],
...
@run_job_using("docker-worker", "yarn", schema=yarn_schema)
def configure_yarn(config, job, taskdesc):
    run = job["run"]
...
def _extract_command(run):
    ...
    yarn_command = ["yarn"] + run.pop("yarn")
    post_yarn_commands = run.pop("post-yarn", [])

    commands = pre_yarn_commands + [yarn_command] + post_yarn_commands
